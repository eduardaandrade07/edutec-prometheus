...
if (pauseDialog && !document.querySelector("#back-to-site-button")) {
    const voltarSiteButton = document.createElement("button");

    voltarSiteButton.id = "back-to-site-button";
    voltarSiteButton.className = "button button--quiet";
    voltarSiteButton.type = "button";
    voltarSiteButton.textContent = "VOLTAR AO SITE";

    voltarSiteButton.addEventListener("click", () => {
    ...
    pauseDialog.appendChild(voltarSiteButton);
}
...
function bindGameEvents() {
    ...
    ui.addEventListener("newcampaign", () => {
        if (ui.els.reportDialog.open) {
            ui.els.reportDialog.close();
        }
    ...
    interaction.addEventListener("interact", (event) => {
        mission.interact(event.detail);
    });
    ...
    gameState.addEventListener("metrics", (event) => {
        ui.updateMetrics(event.detail);
    });
    ...
    world.addEventListener("contextlost", () => {
        ...
        ui.toast(
            "A sessão gráfica foi interrompida. Recarregue a página para continuar.",
            "amber",
            8000
        );
    });
...
function frame(now) {
    ...
    const camera = player.getCamera(
        gameState.settings.reducedEffects
    );
...
try {
    ...
    ui.fail(
        "Não foi possível iniciar o ambiente 3D neste navegador."
    );
}